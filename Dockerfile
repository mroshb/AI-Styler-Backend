# Multi-stage build for AI Styler Backend
...
# Set Go module mode and build the application
ENV GO111MODULE=on
RUN go mod tidy
RUN CGO_ENABLED=0 GOOS=linux go build -o ai-styler .
...
RUN groupadd -g 1001 styler && \
    useradd -u 1001 -g styler -s /bin/bash styler
